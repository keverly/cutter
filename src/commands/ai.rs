...
pub fn run(prompt: &str, base_hint: Option<&str>) -> Result<String> {
    ...
    let child_path = augmented_path(cutter_bin.parent());
...
/// Locate the `claude` binary, honoring an explicit override.
...
/// Build a `PATH` for the Claude child that has `cutter`'s directory (and the
/// common bin dirs) up front, so `cutter`/`git`/`node` resolve regardless of how
/// this process was launched.
fn augmented_path(cutter_dir: Option<&Path>) -> OsString {
    let mut dirs: Vec<PathBuf> = Vec::new();
    if let Some(d) = cutter_dir {
